...
mean_raw
```
Mean number of steps taken per day is `r mean_raw`.

```{r}
median_raw<-median(totalStepsByDay$steps)
median_raw
```
Median number of steps taken per day is `r median_raw`.
...
```
There are `r sum(missingIndex)` missing values in the dataset. 
...
Imputing missing values with m = `r m`
```{r}
activity1<-activity
...
Mean number of steps taken per day is `r mean_afterImput`.

```{r}
median_afterImput<-median(totalStepsByDay1$steps)
median_afterImput
```
Median number of steps taken per day is `r median_afterImput`.
